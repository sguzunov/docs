...
With the declarative model you can provide methods to other apps by creating and attributing interfaces. In the following example, we create a CRM service interface that exposes a `SynchronizeContact()` method:
...
`CRMServiceImpl` holds the actual app logic that will handle Interop calls:
